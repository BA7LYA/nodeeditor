...
int main(int argc, char* argv[])
{
    ...
    const NodeId nodeSource = 0;
    const NodeId nodeResult = 2;

    qInfo() << "========================================";
    qInfo() << "Entering the number " << 33.3 << "to the input node";
    dataFlowGraphModel.delegateModel<NumberSourceDataModel>(nodeSource)
        ->setNumber(33.3);

    qInfo() << "Result of the addiion operation: "
            << dataFlowGraphModel
                   .delegateModel<NumberDisplayDataModel>(nodeResult)
                   ->number();
