...
#include "DynamicPortsModel.hxx"

#include <iterator>
#include <QJsonArray>
#include <QtNodes/ConnectionIdUtils>

#include "PortAddRemoveWidget.hxx"
...
std::unordered_set<ConnectionId> DynamicPortsModel::allConnectionIds(
    const NodeId nodeId
) const
{
...
std::unordered_set<ConnectionId> DynamicPortsModel::connections(
    NodeId    nodeId,
    PortType  portType,
    PortIndex portIndex
) const
...
bool DynamicPortsModel::connectionExists(const ConnectionId connectionId) const
{
    return (_connectivity.find(connectionId) != _connectivity.end());
}

NodeId DynamicPortsModel::addNode(const QString nodeType)
{
...
bool DynamicPortsModel::connectionPossible(const ConnectionId connectionId
) const
{
    return !connectionExists(connectionId);
}

void DynamicPortsModel::addConnection(const ConnectionId connectionId)
{
...
bool DynamicPortsModel::nodeExists(const NodeId nodeId) const
{
...
PortAddRemoveWidget* DynamicPortsModel::widget(NodeId nodeId) const
{
    ...
    {
        _nodeWidgets[nodeId] = new PortAddRemoveWidget(
            0,
            0,
            nodeId,
...
QVariant DynamicPortsModel::nodeData(NodeId nodeId, NodeRole role) const
{
    ...
    case NodeRole::InPortCount: result = _nodePortCounts[nodeId].in; break;

    case NodeRole::OutPortCount: result = _nodePortCounts[nodeId].out; break;
...
bool DynamicPortsModel::setNodeData(
    NodeId   nodeId,
    NodeRole role,
    QVariant value
)
...
QVariant DynamicPortsModel::portData(
    NodeId    nodeId,
    PortType  portType,
    PortIndex portIndex,
    PortRole  role
...
bool DynamicPortsModel::deleteConnection(const ConnectionId connectionId)
{
...
bool DynamicPortsModel::deleteNode(const NodeId nodeId)
{
...
QJsonObject DynamicPortsModel::saveNode(const NodeId nodeId) const
{
...
void DynamicPortsModel::addPort(
    NodeId    nodeId,
    PortType  portType,
    PortIndex portIndex
)
...
void DynamicPortsModel::removePort(
    NodeId    nodeId,
    PortType  portType,
    PortIndex portIndex
)
{
    // STAGE 1.
    // Compute new addresses for the existing connections that are shifted
    // upwards instead of the deleted ports.
    PortIndex first = portIndex;
    PortIndex last  = first;
