...
void PortAddRemoveWidget::populateButtons(
    PortType     portType,
    unsigned int nPorts
)
{
...
void PortAddRemoveWidget::removeButtonGroupFromLayout(
    QVBoxLayout* vbl,
    unsigned int portIndex
)
{
...
void PortAddRemoveWidget::onPlusClicked()
{
    ...
    const int plusButtonIndex = 0;

    PortType  portType;
    PortIndex portIndex;

    // All existing "plus" buttons trigger the same slot. We need to find out
    // which button has been actually clicked.
...
std::pair<PortType, PortIndex> PortAddRemoveWidget::findWhichPortWasClicked(
    QObject*  sender,
...
)
{
        ...
        [&portType, &portIndex, &sender, &buttonIndex](QVBoxLayout* sideLayout)
    {
        for (int i = 0; i < sideLayout->count(); ++i)
        {
            ...
            auto widget
                = static_cast<QWidgetItem*>(hLayout->itemAt(buttonIndex))
                      ->widget();
