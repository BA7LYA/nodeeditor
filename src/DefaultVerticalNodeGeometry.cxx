///
/// @file DefaultVerticalNodeGeometry.cxx
...
DefaultVerticalNodeGeometry::DefaultVerticalNodeGeometry(
...
QSize DefaultVerticalNodeGeometry::size(const NodeId nodeId) const
{
    return _graphModel.nodeData<QSize>(nodeId, NodeRole::Size);
}

void DefaultVerticalNodeGeometry::recomputeSize(const NodeId nodeId) const
{
    ...
    unsigned int totalInPortsWidth
        = nInPorts > 0 ? inPortWidth * nInPorts + _portSpasing * (nInPorts - 1)
                       : 0;
...
QPointF DefaultVerticalNodeGeometry::portPosition(
    const NodeId    nodeId,
...
) const
{
    ...
    case PortType::In:
    {
        unsigned int inPortWidth
            = maxPortsTextAdvance(nodeId, PortType::In) + _portSpasing;

        PortCount nInPorts
            = _graphModel.nodeData<PortCount>(nodeId, NodeRole::InPortCount);

        double x = (size.width() - (nInPorts - 1) * inPortWidth) / 2.0
    ...
    case PortType::Out:
    {
        unsigned int outPortWidth
            = maxPortsTextAdvance(nodeId, PortType::Out) + _portSpasing;
        PortCount nOutPorts
            = _graphModel.nodeData<PortCount>(nodeId, NodeRole::OutPortCount);

        double x = (size.width() - (nOutPorts - 1) * outPortWidth) / 2.0
...
QPointF DefaultVerticalNodeGeometry::portTextPosition(
    const NodeId    nodeId,
    const PortType  portType,
    const PortIndex portIndex
) const
{
    ...
    switch (portType)
    {
    case PortType::In: p.setY(5.0 + rect.height()); break;

    case PortType::Out: p.setY(size.height() - 5.0); break;
...
QRectF DefaultVerticalNodeGeometry::captionRect(const NodeId nodeId) const
{
...
QPointF DefaultVerticalNodeGeometry::captionPosition(const NodeId nodeId) const
{
    QSize size = _graphModel.nodeData<QSize>(nodeId, NodeRole::Size);
...
QPointF DefaultVerticalNodeGeometry::widgetPosition(const NodeId nodeId) const
{
    QSize size = _graphModel.nodeData<QSize>(nodeId, NodeRole::Size);
    ...
    if (auto w = _graphModel.nodeData<QWidget*>(nodeId, NodeRole::Widget))
    {
        ...
        {
            return QPointF(
                _portSpasing + maxPortsTextAdvance(nodeId, PortType::In),
                captionHeight
            );
        }
        else
...
QRect DefaultVerticalNodeGeometry::resizeHandleRect(const NodeId nodeId) const
{
    QSize size = _graphModel.nodeData<QSize>(nodeId, NodeRole::Size);
...
QRectF DefaultVerticalNodeGeometry::portTextRect(
    const NodeId    nodeId,
    const PortType  portType,
    const PortIndex portIndex
) const
...
unsigned int DefaultVerticalNodeGeometry::maxHorizontalPortsExtent(
    const NodeId nodeId
) const
{
    PortCount nInPorts
...
unsigned int DefaultVerticalNodeGeometry::maxPortsTextAdvance(
    const NodeId   nodeId,
    const PortType portType
) const
{
...
unsigned int DefaultVerticalNodeGeometry::portCaptionsHeight(
    const NodeId   nodeId,
    const PortType portType
) const
{
