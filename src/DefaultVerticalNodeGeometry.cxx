...
void DefaultVerticalNodeGeometry::recomputeSize(NodeId const nodeId) const
{
    ...
    unsigned int totalInPortsWidth = nInPorts > 0
                                         ? inPortWidth * nInPorts
                                               + _portSpasing * (nInPorts - 1)
                                         : 0;

    unsigned int totalOutPortsWidth = nOutPorts > 0
                                          ? outPortWidth * nOutPorts
    ...
    NodeId const nodeId, PortType const portType) const
{
    ...
    case PortType::In: {
        ...
        for (PortIndex i = 0; i < nInPorts; ++i) {
            if (_graphModel.portData<bool>(
                    nodeId, PortType::In, i, PortRole::CaptionVisible)) {
                h += _portSpasing;
                break;
            }
        }
    ...
    case PortType::Out: {
        ...
        for (PortIndex i = 0; i < nOutPorts; ++i) {
            if (_graphModel.portData<bool>(
                    nodeId, PortType::Out, i, PortRole::CaptionVisible)) {
                h += _portSpasing;
                break;
            }
        }
