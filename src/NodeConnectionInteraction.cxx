...
bool NodeConnectionInteraction::canConnect(PortIndex *portIndex) const
{
    ...
    QPointF connectionPoint = _cgo.sceneTransform().map(
        _cgo.endPoint(requiredPort));
...
bool NodeConnectionInteraction::tryConnect() const
{
    ...
    ConnectionId newConnectionId = makeCompleteConnectionId(
        incompleteConnectionId, _ngo.nodeId(), targetPortIndex);
...
bool NodeConnectionInteraction::disconnect(PortType portToDisconnect) const
{
    ...
    // Grabs the mouse
    auto const &draftConnection = _scene.makeDraftConnection(
    ...
    PortType portType, PortIndex portIndex) const
{
    ...
    QPointF p = geometry.portScenePosition(
        _ngo.nodeId(), portType, portIndex, _ngo.sceneTransform());
