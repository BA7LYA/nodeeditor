...
TEST_CASE("FlowScene triggers connections created or deleted", "[gui]")
{
    struct MockDataModel : StubNodeDataModel
    {
        ...
        void outputConnectionCreated(const Connection&) override
        {
            outputCreatedCalledCount++;
        }

        void outputConnectionDeleted(const Connection&) override
        {
    ...
    Creation partialCreation {
        "scene.createConnection-by partial",
        [&]
        {
            auto connection
                = scene.createConnection(PortType::Out, fromNode, 0);
    ...
    Deletion sceneDeletion { "scene.deleteConnection", [&](Connection& c) {
    ...
    SECTION("creating a connection")
    {
        ...
        for (const Creation& create : cases)
        {
            SECTION(create.name)
            {
...
TEST_CASE(
    "FlowScene's DataModelRegistry outlives nodes and connections",
    "[asan][gui]"
)
{
    ...
    public:
        ...
        // The reference ensures that we point into the memory that would be
        // free'd if the DataModelRegistry doesn't outlive this node
    ...
    // Introduce a new scope, so that modelsDestroyed will be alive even after
    // the FlowScene is destroyed.
    {
        auto setup = applicationSetup();
